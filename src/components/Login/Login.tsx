  ...
  return(
    <div className="w-full min-h-screen relative">
      <form onSubmit={handleSubmit(handleLogin)} className='w-[85%] md:w-[65%] lg:w-[30%] m-auto rounded-2xl flex flex-col p-10 items-center gap-8 absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-[rgb(240, 242, 245)] shadow-2xl'>
        <Image src={Logo} width={150} height={150} alt="logo" className="w-fit object-contain h-fit" />
        ...
        <fieldset className='w-full flex flex-col gap-6 m-auto items-center'>
          <div className="w-full flex flex-col gap-2">
            <input {...register('email')} type='email' placeholder='E-mail' className='w-full px-2 py-2 text-black text-sm font-normal rounded-lg border-2 border-grayBorder focus:outline-redBorder'  />
            {errors.email && <span className="text-sm text-red font-normal">{errors.email.message}</span>}
          </div>
          <div className="w-full flex flex-col gap-2">
            <input {...register('password')} type='password' placeholder='Senha' className='w-full px-2 py-2 text-black text-sm font-normal rounded-lg border-2 border-grayBorder focus:outline-redBorder'  />
